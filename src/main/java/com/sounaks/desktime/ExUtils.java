	...
	public static String formatUptime(Duration duration, String pattern)
	{
		...
		for(String val : arr)
		{
			if (outOfQuote)
			{
				...
				if (val.contains("HH")) {
					upTimeFormatted += val.replace("HH", String.valueOf(duration.toHours()));
				}
				else if (val.contains("mm"))
				{
					upTimeFormatted += val.replace("mm", String.valueOf(duration.toMinutes() % 60));
				}
				else if (val.contains("ss"))
				{
					upTimeFormatted += val.replace("ss", String.valueOf((duration.getSeconds() % 60) % 60));
				}
			}
			else
			{
