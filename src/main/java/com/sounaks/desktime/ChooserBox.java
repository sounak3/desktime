...
public class ChooserBox extends JDialog implements ActionListener, ItemListener, ListSelectionListener
{
	...
	public void initialize(InitInfo initinfo, Vector <TimeBean>alarmData) //Called by->constructor->showDialog
	{
		...
		hSymbol.setText(s1.substring(s1.indexOf("HH'") + 3, s1.indexOf(",", s1.indexOf("HH'") + 3)));
		mSymbol.setText(s1.substring(s1.indexOf("mm'") + 3, s1.indexOf(",", s1.indexOf("mm'") + 3)));
		sSymbol.setText(s1.substring(s1.indexOf("ss'") + 3, s1.indexOf("'", s1.indexOf("ss'") + 3)));
		uSymbol.setText(s1.substring(s1.indexOf("'") + 1, s1.indexOf("'", s1.indexOf("'") + 1)));
		data=alarmData;
	...
	public void actionPerformed(ActionEvent actionevent)
	{
		...
		else if(comm.equals("Browse..."))
		{
			...
			try
			{
				ii=fchoose.showOpenDialog(this);
				switch(ii)
				{
